...
The website will be available at `http://localhost:3000` and PostgreSQL at `localhost:15432`.
...
The CMS is an application-level `/admin` login, not Caddy browser Basic Auth. It is disabled by default and fails closed unless `ADMIN_CMS_ENABLED=true`. Set one explicit `ADMIN_AUTH_MODE`: `magic_link` for a Resend magic link sent to one exact email address, or `password` for a primary username and an Argon2id password hash, plus optional extra users. The login form and routes fail closed if settings for the selected mode are incomplete or credentials for both modes are present.
...
| `ADMIN_USERNAME` | Required only for `password`: 1–100 ASCII letters, digits, `.`, `_`, `-`, or `@` (an e-mail address may be used as the username). |
| `ADMIN_PASSWORD_HASH` | Required only for `password`: Argon2id PHC hash. Never store a plaintext password. |
| `ADMIN_EXTRA_USERS` | Optional, password mode only: newline-separated `username:argon2idHash` entries for additional administrators. Each line is `username` followed by `:` then the PHC hash; the hash contains no `:` so the first `:` splits the pair. Duplicate usernames or a duplicated primary `ADMIN_USERNAME` are rejected. |
...
Generate each password hash on a trusted machine using Argon2id with at least 64 MiB memory and three iterations. Store only the complete PHC string as an encrypted Vercel/environment variable. In `.env.production`, wrap the complete hash in single quotes, for example `ADMIN_PASSWORD_HASH='$argon2id$...'`, so Docker Compose does not interpolate `$`. An `ADMIN_EXTRA_USERS` value with embedded newlines must be quoted so Compose keeps each line separate; a `:` alone is the delimiter, so an extra user whose username is an e-mail address works because `@` needs no escaping. Assigning any user's hash is what lets a person log in, and the current password is what they enter. Changing any user's password hash (primary or extra) invalidates all password-mode sessions except the currently active one. Password login retains the per-IP database limiter and adds a wider account limiter; both reset after 15 minutes.
...
- `RESEND_API_KEY` (or the supported legacy name `RESEND_TOKEN`) and `CONTACT_FORM_RESEND_FROM` are optional and must either both be configured or both be empty; `CONTACT_FORM_RECIPIENT` optionally enables the owner notification; the sender domain used by `CONTACT_FORM_RESEND_FROM` must be verified in Resend
...
Each tab should have a header row matching the fields submitted for that lesson type. The `Junior Voice` tab uses the child columns: `Data zgłoszenia`, `Imię i nazwisko dziecka`, `Data urodzenia dziecka`, `Przedszkole, do którego uczęszcza dziecko`, `Grupa`, `Ulica i numer`, `Kod pocztowy`, `Miasto`, `Numer telefonu`, `Adres e-mail`, `Zgoda na zapłatę`, and `Wizerunek dziecka`. The `Studio Wokalne` and `Rehabilitacja zaburzeń głosu` tabs omit child-only columns: `Data zgłoszenia`, `Imię i nazwisko`, `Data urodzenia`, `Ulica i numer`, `Kod pocztowy`, `Miasto`, `Numer telefonu`, `Adres e-mail`, and `Wizerunek uczestnika`. PostgreSQL remains the primary store. A Sheets API failure does not reject the form submission. Because this creates a second copy of personal data, apply the same retention and access controls to the spreadsheet.
